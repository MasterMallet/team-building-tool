...
1. 受付画面で学年を選択（1-6年生）
2. 「登録する」ボタンで登録
3. 表示された受付番号のリストバンドを渡す
4. 次の参加者の学年を選択して登録を繰り返す

受付番号は1番から昇順に付与され、削除や再読み込みでは再利用されません。全データのリセットで1番に戻ります。参加者一覧・チーム分け・CSV・印刷には同じ受付番号が表示されます。既存の名前付きデータには保存順で番号が付きます。CSVは「番号,学年」形式で、旧形式「番号,名前,学年」も読み込めます。インポート時はCSVの番号を引き継ぎます。
