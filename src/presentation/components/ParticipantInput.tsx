...
export const ParticipantInput: React.FC<Props> = ({
  participants,
  name,
  grade,
  onNameChange,
    ...
    <div className="bg-gradient-to-br from-purple-500 to-purple-700 rounded-2xl p-6 text-white">
      ...
      <div className="mt-4 max-h-64 overflow-y-auto bg-white/20 rounded-lg p-2">
        {participants.map(p => (
          <div
            key={p.id}
            className="flex justify-between items-center bg-white/90 text-gray-800 rounded-lg p-2 mb-2"
          >
            <span>
              {p.name} ({p.grade}年)
            </span>
            <button onClick={() => onRemove(p.id)} className="text-red-500 hover:text-red-700">
              <Trash2 size={16} />
            </button>
          </div>
        ))}
